...
# clobbers the symlink with a plain file, `rm` it and rebuild to restore it.
{
  ...
  # The core service (the root daemon that grabs keyboards) occasionally fails
  # to resume grabbing after sleep — remaps silently stop applying until it's
  # kickstarted (observed 2026-09-05: last log line was system_will_sleep, no
  # re-grab after wake). Allow that one exact command without a password so
  # `karabiner-restart` (below) is a one-shot fix. No dot in the filename:
  # sudo skips sudoers.d files containing '.'.
  environment.etc."sudoers.d/karabiner".text = ''
    ${config.system.primaryUser} ALL=(root) NOPASSWD: /bin/launchctl kickstart -k system/org.pqrs.service.daemon.Karabiner-Core-Service
  '';

  home-manager.users.${config.system.primaryUser} = {
    programs.fish.functions.karabiner-restart = {
      description = "Restart the Karabiner core service (fixes remaps not applying after sleep)";
      body = ''
        sudo /bin/launchctl kickstart -k system/org.pqrs.service.daemon.Karabiner-Core-Service
        echo "Karabiner core service restarted; keyboards re-grab within ~10s."
      '';
    };
